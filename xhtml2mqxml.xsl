...
<xsl:variable name="metadata" select="//x:html/x:head/x:wordmeta"/>
<xsl:variable name="courseID" select="$metadata/x:meta[@name='moodleCourseID']/@content" />
<xsl:variable name="moodleRelease" select="$metadata/x:meta[@name='moodleRelease']/@content" />
<xsl:variable name="moodleReleaseNumber">
	...
	<xsl:when test="starts-with($moodleRelease, '2.8')"><xsl:text>28</xsl:text></xsl:when>
	<xsl:otherwise><xsl:text>26</xsl:text></xsl:otherwise> <!-- 2.5, 2.6 and higher are considered the same for the moment -->
	</xsl:choose>
</xsl:variable>
...
<xsl:template match="/">
    ...
    -->
		<xsl:value-of select="'&#x0a;'"/>
		<xsl:comment>Course (ID): <xsl:value-of select="concat(//x:html/x:body/x:div/x:p[@class = 'Title'], ' (', $courseID, ')')"/></xsl:comment>
		<xsl:value-of select="'&#x0a;'"/>
		...
		<xsl:value-of select="'&#x0a;'"/>
		<!-- 2 cases to handle: a) all questions; b) 1 preview question -->
		<xsl:choose>
...
<xsl:template name="itemStem">
    ...
    <xsl:comment><xsl:value-of select="concat('$generalfeedback_label:', $generalfeedback_label)"/></xsl:comment>
		<xsl:call-template name="rich_text_content">
...
<xsl:template match="x:strong|x:b|x:em[@class = 'bold']" mode="cloze">
	...
	<xsl:when test="contains($correct_option, $cloze_mc_keyword1) or contains($correct_option, $cloze_mc_keyword2) or contains($correct_option, $cloze_mch_keyword1) or contains($correct_option, $cloze_mch_keyword2) or contains($correct_option, $cloze_mcv_keyword1) or contains($correct_option, $cloze_mcv_keyword2)">
		<!-- MC subquestion contains Moodle keywords, so don't attempt to process it further -->
		<xsl:value-of select="$correct_option"/>
	</xsl:when>
		<xsl:when test="starts-with($correct_option, '%') or starts-with($correct_option, $cloze_correct_prefix2)">
	...
	<xsl:otherwise>
		...
		<xsl:variable name="other_mc_answers">
			<xsl:for-each select="ancestor::x:p//strong|ancestor::x:ul//strong|ancestor::x:ol//strong">
...
<xsl:template match="x:em[@class = 'italic']|x:i" mode="cloze">
	...
	<xsl:when test="starts-with($correct_option, '%')">
...
<xsl:template match="x:u|x:span[@style and contains(@style, 'underline')]" mode="cloze">
	...
	<xsl:when test="contains($correct_option, '%')">
...
<xsl:template name="format_cloze_answer">
	...
	<xsl:if test="not(starts-with($answer_string, '%')) and not(starts-with($answer_string, $cloze_wildcard_wrong))">
...
<xsl:template name="get_cloze_distractor_answer_string">
	...
	<xsl:for-each select="$table_root/x:tr[count(x:td) = 4]">
		<!-- Distractor row column 2 is the text -->
		<xsl:variable name="distractor_text">
			<xsl:if test="normalize-space(x:td[2]) != '&#160;'">
				<xsl:value-of select="normalize-space(x:td[2])"/>
			</xsl:if>
		</xsl:variable>
		<!-- Distractor row column 3 is feedback -->
		<xsl:variable name="distractor_feedback">
			<xsl:if test="normalize-space(x:td[3]) != '&#160;'">
				<xsl:value-of select="normalize-space(x:td[3])"/>
			</xsl:if>
		</xsl:variable>
		<!-- Distractor row column 4 is a grade, generally blank or 0 (%) -->
		<xsl:variable name="distractor_grade">
			<xsl:choose>
			<xsl:when test="normalize-space(x:td[4]) != '' and normalize-space(x:td[4]) != '&#160;'">
				<xsl:value-of select="concat('%', normalize-space(x:td[4]), '%')"/>
			</xsl:when>
			<xsl:otherwise>
